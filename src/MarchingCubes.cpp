...
//_____________________________________________________________________________
// main algorithm
void MarchingCubes::run(double grid_size,double &cell_vol, real iso )
//-----------------------------------------------------------------------------
{
  ...
  for( _i = 0 ; _i < _size_x-1 ; _i++ )
  {
    ...
    for( int p = 0 ; p < 8 ; ++p )
    {
	  ...
	  if( _cube[p] > 0 ) {_lut_entry += 1 << p ; num_positive++; m_positive_index.push_back(p);}
	  else
		  m_nonpositive_index.push_back(p);
    }

	if(num_positive==1 || num_positive==2 || num_positive==3 || num_positive==5)
		int t_break = 0;
	if(num_positive>4)
		m_flipped = true;
	else
	...
	m_grid_points.resize(8);
	m_grid_points[0] = CVector3d(0,        0,        0        );
	m_grid_points[1] = CVector3d(grid_size,0,        0        );
	m_grid_points[2] = CVector3d(grid_size,grid_size,0        );
	m_grid_points[3] = CVector3d(0,        grid_size,0        );
	m_grid_points[4] = CVector3d(0,        0,        grid_size);
	m_grid_points[5] = CVector3d(grid_size,0,        grid_size);
	m_grid_points[6] = CVector3d(grid_size,grid_size,grid_size);
	m_grid_points[7] = CVector3d(0,        grid_size,grid_size);

    process_cube(grid_size,cell_vol);
  }
...
void MarchingCubes::init_all (std::vector<std::vector<std::vector<double> > > &data, float iso)
//-----------------------------------------------------------------------------
{
  init_temps() ;
  ...
  for( int k = 0 ; k < size_z(); k++ )
	  for( int j = 0 ; j <  size_y() ; j++ )
		  for( int i = 0 ; i <  size_x() ; i++ )
			  set_data( (float) ((data[i][j][k]-iso)),i,j,k ) ;
}
//_____________________________________________________________________________
...
double MarchingCubes::compute_vol_case_1_2_5_8_9_11_14(double grid_length){
	int num_points;
	if(m_flipped)
		num_points = _nverts + m_nonpositive_index.size();
	else
		num_points = _nverts + m_positive_index.size();

	double* current_pos = new double[num_points*3];
	for(int i=0;i<_nverts;i++){
		current_pos[3*i  ] = grid_length*_vertices[i].x;
		current_pos[3*i+1] = grid_length*_vertices[i].y;
		current_pos[3*i+2] = grid_length*_vertices[i].z;
	}

	if(m_flipped){
		for(int i=0;i<m_nonpositive_index.size();i++){
			current_pos[3*(_nverts+i)  ] = m_grid_points[m_nonpositive_index[i]][0];
			current_pos[3*(_nverts+i)+1] = m_grid_points[m_nonpositive_index[i]][1];
			current_pos[3*(_nverts+i)+2] = m_grid_points[m_nonpositive_index[i]][2];
		}
	}
	else{
		for(int i=0;i<m_positive_index.size();i++){
			current_pos[3*(_nverts+i)  ] = m_grid_points[m_positive_index[i]][0];
			current_pos[3*(_nverts+i)+1] = m_grid_points[m_positive_index[i]][1];
			current_pos[3*(_nverts+i)+2] = m_grid_points[m_positive_index[i]][2];
		}
	}
    ...
    ad.push_back(3);
    ad.push_back(num_points);
    matlab::data::TypedArray<double> P = factory.createArray(ad, current_pos, current_pos+num_points*3);
    
    _m->setVariable(u"P", std::move(P));
    _m->eval(u"P=P';");
    _m->eval(u"[k,v]=convhull(P(:,1),P(:,2),P(:,3));");
    matlab::data::TypedArray<double> current_v = _m->getVariable(u"v");

    current_vol = current_v[0];
...
double MarchingCubes::compute_vol_case_3_4_6_7_10_12_13(double grid_length){
	int num_points;
	if(m_flipped)
		num_points = _nverts + m_positive_index.size();
	else
	...
	double* current_pos = new double[36];
	for(int i=0;i<_nverts;i++){
		current_pos[3*i  ] = grid_length*_vertices[i].x;
		current_pos[3*i+1] = grid_length*_vertices[i].y;
		current_pos[3*i+2] = grid_length*_vertices[i].z;
	...
	return (pow(grid_length,3)-current_vol);
}

double MarchingCubes::compute_tet_vol(CVector3d *pos){
	double tmp = (pos[0]-pos[3]).Dot((pos[1]-pos[3]).Cross(pos[2]-pos[3]));
	return fabs(tmp);
}

//_____________________________________________________________________________
// Process a unit cube
void MarchingCubes::process_cube(double grid_size,double &cell_vol)
//-----------------------------------------------------------------------------
{
  int   v12 = -1 ;
  ...
  double current_vol;
  //meaning all non-positive points
  switch( _case )
  {
  case  0 :
	  current_vol = 0.0;
  ...
  case  1 :
	...
	current_vol = compute_vol_case_1_2_5_8_9_11_14(grid_size);
    break ;
  ...
  case  3 :
	...
	current_vol = compute_vol_case_3_4_6_7_10_12_13(grid_size);
    break ;
  ...
  if(m_flipped)
	  cell_vol = current_vol;
  else
	  cell_vol = pow(grid_size,3) - current_vol;
...
//_____________________________________________________________________________
// write off file
void MarchingCubes::writeOFF(const char *fn , double org_x,double org_y, double org_z, double d)
//-----------------------------------------------------------------------------
{
  FILE *fp = fopen( fn, "w" ) ;
  int   i ;
  ...
  //_vertices[i].x _vertices[i].y _vertices[i].z is coordinates in cells. X; 0 - (NX-1)  Y: 0 - (NY-1) Z: 0 - (NZ-1)
  for ( i = 0; i < _nverts; i++ )
	fprintf( fp, "%f %f %f\n", org_x + d * _vertices[i].x, org_y + d * _vertices[i].y, org_z + d * _vertices[i].z ) ;
	
	//print normals with vertices
