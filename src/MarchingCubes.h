...
public :
  ...
  void init_all   (std::vector<std::vector<std::vector<double> > > &data, float iso) ;
  /** clears temporary structures : the grid and the main */
  void clean_temps() ;
  /** clears all structures : the temporary structures and the mesh buffers */
  void clean_all  () ;

  double compute_vol_case_1_2_5_8_9_11_14(double grid_length);
  double compute_vol_case_3_4_6_7_10_12_13(double grid_length);
  double compute_tet_vol(CVector3d *pos);
...
public :
  ...
  /**
   * OFF FILE fomat of the generated mesh
   * \param fn  name of the OFF file to create
   */
	void writeOFF ( const char *fn, double org_x,double org_y, double org_z, double d) ;
...
public :
   ...
   * \param iso isovalue
   */
    void run( double grid_size,double &cell_vol,real iso = (real)0.0);
             //,std::unique_ptr<matlab::engine::MATLABEngine> m = matlab::engine::startMATLAB()) ;
//matlab::engine::FutureResult<std::unique_ptr<matlab::engine::MATLABEngine>> m
    
protected :
  /** tessellates one cube */
  void process_cube (double grid_size,double &cell_vol)             ;
...
protected :
  ...
  std::vector<int> m_positive_index;
  std::vector<int> m_nonpositive_index;
  std::vector<CVector3d> m_grid_points;
  ...
  std::unique_ptr<matlab::engine::MATLABEngine> _m; //Added by Emily
